...
$diet = $pdo->prepare("SELECT * FROM diets WHERE id=?");
$diet->execute([$diet_id]);
$diet = $diet->fetch();
...
if ($id) {
    $stmt = $pdo->prepare("SELECT * FROM meals WHERE id=? AND diet_id=?");
...
?>
<div class="card rise-up hover-lift">
  <div class="card-body">
    <h4 class="mb-3"><?= $id ? 'Editar Comida' : 'Nueva Comida' ?></h4>
    ...
    <form method="post" novalidate>
      ...
      <div class="mt-3 d-flex gap-2">
        <button class="btn btn-success btn-ink" type="submit">Guardar</button>
        <a class="btn btn-secondary btn-ink" href="meals.php?diet_id=<?= (int)$diet_id ?>">Cancelar</a>
      </div>
    </form>
  </div>
</div>
